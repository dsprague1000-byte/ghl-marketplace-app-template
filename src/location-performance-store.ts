...
export async function correctLocationReport(d:any){const payload=canonical(d.input),ph=hash({payload,reason:d.reason}),c=await db().connect();try{await c.query("BEGIN");
 ...
 if(isoDate(b.week_start_date)!==payload.week_start_date)throw Object.assign(new Error("Correction cannot change the report week"),{statusCode:400});const nv=d.expectedVersion+1;
