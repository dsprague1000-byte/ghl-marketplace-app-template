        ...
        <section class="empty-state">
          ...
          <form class="form-grid" @submit.prevent="provisionAssignment">
            <label class="full-span">
              ...
              <input v-model.trim="provisionRecordId" required />
            </label>
      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <section v-if="activeView === 'overview'" class="panel-grid">
            ...
            <article v-if="locationGoal.goal" class="wide-card goal-card">
              <div class="section-heading">
                <div>
                  <p class="eyebrow">Location Goal</p>
                  <h2>{{ locationGoal.month }}</h2>
                </div>
                <strong>
                  {{ locationGoal.goal.memberships_target }} memberships ·
                  {{ percent(locationGoal.goal.conversion_target) }} conversion
                </strong>
              </div>
              <div class="goal-grid">
                <div>
                  <span>Actual sold</span>
                  <strong>{{ locationGoal.pace?.actual || 0 }}</strong>
                </div>
                <div>
                  <span>Expected by today</span>
                  <strong>{{ locationGoal.pace?.expectedToDate || 0 }}</strong>
                </div>
                <div>
                  <span>Variance</span>
                  <strong>{{ signed(locationGoal.pace?.variance) }}</strong>
                </div>
                <div>
                  <span>Goal progress</span>
                  <strong>{{ percent(locationGoal.pace?.percentOfGoal) }}</strong>
                </div>
              </div>
            </article>

            <article v-else class="wide-card">
              <p class="eyebrow">Location Goal</p>
              <p class="muted">No location goal has been set for {{ selectedMonth }}.</p>
            </article>
            ...
            <article class="wide-card">
              <div class="section-heading">
                <div>
                  ...
                  <h2>{{ rollup.month || selectedMonth }}</h2>
                </div>
                <input v-model="selectedMonth" type="month" @change="changeMonth" />
              </div>
              <table v-if="rollup.sellers?.length" class="data-table">
                <thead>
                  <tr><th>Seller</th><th>Opportunities</th><th>Sold</th><th>Conversion</th><th>Pending</th></tr>
                </thead>
                <tbody>
          ...
          <section v-if="activeView === 'goals'" class="wide-card">
            <p class="eyebrow">P026A · Location Management</p>
            <h2>Set Monthly Location Goal</h2>
            <form class="form-grid" @submit.prevent="saveLocationGoal">
              ...
              <div class="full-span form-actions">
                <button class="primary-button" :disabled="saving">{{ saving ? 'Saving…' : 'Save location goal' }}</button>
                <span>{{ actionMessage }}</span>
              </div>
            </form>
          </section>
          ...
          <section v-if="activeView === 'submit'" class="wide-card">
            ...
            <form class="form-grid" @submit.prevent="submitShift">
              ...
              <div class="full-span form-actions">
                <button class="primary-button" :disabled="saving">Submit shift</button>
                <span>{{ actionMessage }}</span>
              </div>
            </form>
          </section>
          ...
          <section v-if="activeView === 'provision'" class="wide-card">
            ...
            <form class="form-grid" @submit.prevent="provisionAssignment">
              ...
              <div class="full-span form-actions">
                <button class="primary-button" :disabled="saving">Provision assignment</button>
                <span>{{ actionMessage }}</span>
              </div>
            </form>
          </section>
...
export default {
  ...
  methods: {
    ...
    async loadLocationGoal() {
      this.locationGoal = await this.api('/goals/location', { month: this.selectedMonth })
    },
    async saveLocationGoal() {
      this.saving = true
      this.actionMessage = ''
      try {
        await this.api('/goals/location/set', {
          month: this.goalForm.month,
          membershipsTarget: this.goalForm.membershipsTarget,
          conversionTarget: Number(this.goalForm.conversionPercent) / 100
        })
    ...
    async loadShiftLogs() {
      this.shiftLogs = (await this.api('/seller/shift-logs')).logs || []
    },
    async loadReviewQueue() {
      this.reviewQueue = (await this.api('/manager/review-queue')).logs || []
    },
    async submitShift() {
      this.saving = true
      try {
    ...
    async provisionAssignment() {
      ...
      try {
        ...
        this.actionMessage = result.mode === 'self'
          ? `MPP access activated for ${result.assignment.assignment_name}.`
          : `Provisioned ${result.assignment.assignment_name}.`
    ...
    rate(sold, opportunities) {
      return Number(opportunities) > 0 ? Number(sold) / Number(opportunities) : 0
    },
...
@media (max-width: 760px) {
  ...
  .panel-grid, .goal-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
