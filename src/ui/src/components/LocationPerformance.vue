...
export default {
  ...
  methods:{
    ...
    async loadPrefill(){try{const x=await this.api('/location-performance/reports/prefill',this.payload({weekStart:this.form.weekStart}));this.prefill=x.prefill;if(x.prefill){this.form.beginningActiveMemberships=x.prefill.prefilledBeginningActiveMemberships;this.message='Beginning Active Memberships prefilled from the prior Ending value. Confirm or correct against the source.'}}catch(e){this.message='Error: '+e.message}},
